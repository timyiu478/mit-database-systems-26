...
func setUpHashJoin(t *testing.T) Executor {
	...
	return NewHashJoinExecutor(planner.NewHashJoinNode(
		leftScan.PlanNode(),
		rightScan.PlanNode(),
		[]planner.Expr{leftKey},
		[]planner.Expr{rightKey},
	), leftScan, rightScan)
}

// TestHashJoin_BasicEqui tests an equality join (Sales.id = Shipments.sale_id).
func TestHashJoin_BasicEqui(t *testing.T) {
...
// TestHashJoin_Empty verifies that the join handles empty tables correctly.
func TestHashJoin_Empty(t *testing.T) {
	...
	joinTree := NewHashJoinExecutor(planner.NewHashJoinNode(
		leftScan.PlanNode(),
		rightScan.PlanNode(),
		[]planner.Expr{leftKey},
		[]planner.Expr{rightKey},
...
// TestHashJoin_Nulls verifies that NULLs in the join key do not match.
func TestHashJoin_Nulls(t *testing.T) {
	bp := storage.NewBufferPool(20, storage.NewDiskStorageManager(t.TempDir()), storage.NoopLogManager{})
	sales := setupSalesTable(t, bp)
	shipment := setupShipmentTable(t, bp)
...
// TestHashJoin_Duplicates verifies that the join logic handles a mix of duplicate and unique keys.
func TestHashJoin_Duplicates(t *testing.T) {
	bp := storage.NewBufferPool(20, storage.NewDiskStorageManager(t.TempDir()), storage.NoopLogManager{})
	sales := setupSalesTable(t, bp)
	shipment := setupShipmentTable(t, bp)
...
// TestHashJoin_Large tests the join on a dataset larger than a single page.
func TestHashJoin_Large(t *testing.T) {
	numTuples := 2000
...
// TestHashJoin_MultiKey verifies that the join works with composite keys (e.g. JOIN ON a=x AND b=y)
